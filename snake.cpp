...
bool randomChance(double p) { return (rand() / (RAND_MAX + 1.0)) < p; }

bool in_bounds(pointT pos, const gameT &game) {
  return pos.row >= 0 && pos.col >= 0 && pos.row < game.numRows &&
         pos.col < game.numCols;
}
...
void next_move(gameT &game) {
  ...
  if (crash(next, game) || randomChance(TURN_RATE)) {
    ...
    bool left_crash =
        crash(next_pos(game.snake.front(), left_dx, left_dy), game);
    bool right_crash =
        crash(next_pos(game.snake.front(), right_dx, right_dy), game);
